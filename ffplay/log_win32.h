...
/*to file*/
#define DEBUG_OUTPUT_MODE		DEBUG_OUTPUT_FILE	/* output mode */
...
#define LOG(format, ...) LOG_D(format, __VA_ARGS__)
#elif DEBUG_OUTPUT_MODE == DEBUG_OUTPUT_FILE  /*这种写文件*/

#define LOGD(format, ...) {log_to_file(DEBUG_FORMAT format, DEBUG_D_TAG, __FILENAME__, __LINE__, __func__, __VA_ARGS__);}
#define LOGE(format, ...) {log_to_file(DEBUG_FORMAT format, DEBUG_E_TAG, __FILENAME__, __LINE__, __func__, __VA_ARGS__);}
#define LOGW(format, ...) {log_to_file(DEBUG_FORMAT format, DEBUG_W_TAG, __FILENAME__, __LINE__, __func__, __VA_ARGS__);}
#define LOGI(format, ...) {log_to_file(DEBUG_FORMAT format, DEBUG_I_TAG, __FILENAME__, __LINE__, __func__, __VA_ARGS__);}
#define LOG(format, ...) LOG_D(format, __VA_ARGS__)
#define DEBUG_FILENAME			"debug.log"  /*日志文件*/
// write a log to file
void log_to_file(const char * format, ...);

#elif DEBUG_OUTPUT_MODE == DEBUG_OUTPUT_CON  /*这种是默认*/
#define DEBUG_STREAM			stderr
